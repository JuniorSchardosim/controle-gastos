...
async function adicionarDespesa() {
    ...
    const arquivoInput = document.getElementById('comprovante-despesa');
    ...
    if (arquivoInput.files.length > 0) {
        ...
        // Converter para Base64 para salvar no Firestore (limite de 1MB por documento)
        comprovante = await new Promise((resolve) => {
...
function renderizarDespesas() {
    ...
    lista.innerHTML = despesas.map((despesa, index) => {
        ...
        } else if (despesa.totalParcelas > 1) {
            const atual = despesa.parcelaAtual || 1;
            const total = despesa.totalParcelas;
            badgeParcela = `<span class="prioridade-badge prioridade-media">${atual}/${total}</span>`;
            infoExtra = `Parcela ${atual} de ${total}`;
        } else {
        ...
        if (despesa.comprovante) {
            linkComprovante = `<a href="${despesa.comprovante}" target="_blank" style="color: var(--primary); font-size: 0.8rem; text-decoration: none; display: block; margin-top: 5px;">📄 Ver Comprovante</a>`;
        }
...
function fecharMes() {
    ...
    // Lógica para replicar despesas para o próximo mês
    const novasDespesas = [];
    despesas.forEach(d => {
        if (d.fixa) {
            // Despesa fixa: repete sempre, limpando o comprovante do mês anterior
            novasDespesas.push({
                ...d,
                comprovante: null // O comprovante é específico de cada mês
            });
        } else if (d.totalParcelas > 1) {
            // Despesa parcelada: verifica se ainda há parcelas
            const proximaParcela = (d.parcelaAtual || 1) + 1;
            if (proximaParcela <= d.totalParcelas) {
                novasDespesas.push({
                    ...d,
    ...
    renderizarHistorico();
    salvarDados();
    alert('Mês fechado com sucesso! Despesas fixas e parcelas restantes foram movidas para o próximo mês.');
}
